...
if ( class_exists( 'BP_Group_Extension' ) ) : // Recommended, to prevent problems during upgrade or when Groups are disabled
	...
	class LS_BPFB_Videos_Tab_Extension extends BP_Group_Extension {
		...
		function __construct() {
			...
			if ( $bp->groups->current_group ) {
				...
				$this->count_videos      = ! empty( $activities_template->activity_count ) ? $activities_template->activity_count : '0';
				$this->nav_item_name     = $this->name . ' <span>' . $this->count_videos . '</span>';
		...
		function display( $group_id = null ) {
			...
			<div class="info-group">
				...
				<?php if ( bp_group_is_member() ) : ?>
					<div class="generic-button group-button public"><a href="<?php bp_group_permalink(); ?>#whats-new-form" class="generic-button" id="ls_bpfb_add_video"><?php _e( 'Add New', 'buddypress' ); ?></a></div>
				<?php endif; ?>

				<?php
				do_action( 'bp_before_activity_loop' );
				if ( $this->count_videos > 0 ) :
					if ( bp_has_activities( 'primary_id=' . $bp->groups->current_group->id . '&object=groups&action=activity_update&search_terms=bpfb_video' ) ) :
